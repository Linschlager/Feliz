...
let private run (launchOptions: LaunchOptions) (path: string) =
    ...
    task {
        printfn "Going to launch the browser"
        use! browser = Puppeteer.LaunchAsync(launchOptions)
        printfn "Got the browser"
        printfn "Got the context"
        use! page = browser.NewPageAsync()
        printfn ""
        printfn "Navigating to http://localhost:%d/index.html" port
        let! _ = page.GoToAsync(sprintf "http://localhost:%d/index.html" port)
        ...
        let! _ = page.EvaluateExpressionAsync(toArrayFunction)
        let! _ = page.EvaluateExpressionAsync(getResultsFunctions)
        ...
        let! _ = page.WaitForExpressionAsync("document.getElementsByClassName('executing').length === 0", waitOptions)
        ...
        let! passedTestsCount = page.EvaluateExpressionAsync<int>(passingTests)
        let failingTests = "document.getElementsByClassName('failed').length"
        let! failedTestsCount = page.EvaluateExpressionAsync<int>(failingTests)
        let pendingTests = "document.getElementsByClassName('pending').length"
        let! pendingTestsCount = page.EvaluateExpressionAsync<int>(pendingTests)
        let! testResults = page.EvaluateExpressionAsync<string[][]>("window.getTests()")
        printfn ""
        printfn "========== SUMMARY =========="
        printfn ""
...
let private isAbsolute (path: string) =
...
let private validatePath path =
    if not (isAbsolute path) && Environment.OSVersion.Platform = PlatformID.Win32NT then
...
let runTests (path: string) : System.Threading.Tasks.Task<int> =
    ...
    | Ok() ->
        ...
        printfn "Chromium browser downloaded to %s" (installedBrowser.GetExecutablePath())

        let launchOptions =
            LaunchOptions(Headless = true, ExecutablePath = installedBrowser.GetExecutablePath())
